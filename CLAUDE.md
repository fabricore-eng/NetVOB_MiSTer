...
- **Shared dev hub (MULTI-SESSION — read first):** Shared MiSTer tooling + lessons + the
  build/device-lock protocol live in **`~/Dev/fabricore/tools`** (`PROTOCOL.md`); `dell` + the
  test HW are **shared with the 573 session** — honor the locks + shared board. Build via
  `~/Dev/fabricore/tools/tools/dell_build.sh` (`DELL_PROJECT=dvd DELL_TARGET=mpeg2fpga
